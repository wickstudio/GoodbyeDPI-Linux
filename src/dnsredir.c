/*
 * DNS UDP Connection Tracker for GoodbyeDPI-Linux
 *
...
/**
 * Flush DNS resolver cache on Linux
 */
void flush_dns_cache(void) {
    /* Try systemd-resolved first, then nscd */
    if (system("systemd-resolve --flush-caches 2>/dev/null || "
               "resolvectl flush-caches 2>/dev/null || "
               "systemctl restart nscd 2>/dev/null || "
               "true") != 0) {
        printf("Note: DNS cache flush may not have worked on this system\n");
    }
}
 ...
 */
static inline void fill_key_data(char *key, const uint8_t is_ipv6, const uint32_t srcip[4],
 ...
 */
static inline void fill_data_from_key(uint8_t *is_ipv6, uint32_t srcip[4], uint16_t *srcport,
                                      const char *key)
{
    if (key[0] == '6') {
        *is_ipv6 = 1;
 ...
 */
static inline void construct_key(const uint32_t srcip[4], const uint16_t srcport,
 ...
 */
static inline void deconstruct_key(const char *key, const udp_connrecord_t *connrecord,
