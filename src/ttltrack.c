/**
 * TCP (TTL) Connection Tracker for GoodbyeDPI-Linux
 *
...
static inline void fill_data_from_key(uint8_t *is_ipv6, uint32_t srcip[4], uint32_t dstip[4],
...
static inline void construct_key(const uint32_t srcip[4], const uint32_t dstip[4],
...
static inline void deconstruct_key(const char *key, const tcp_connrecord_t *connrecord,
